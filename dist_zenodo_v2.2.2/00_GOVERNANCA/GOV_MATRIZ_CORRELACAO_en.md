---
author:
- affiliation: Universidade de Brasília / Núcleo Takwara
  name: Takwara, Fabio Resck
  orcid: 0000-0001-8815-3885
date: '2026-03-04'
doi: 10.5281/zenodo.18882784
language: en
license: CC BY 4.0
series: Operational Governance
title: 'Correlation Matrix: GDocs vs GitHub'
translations:
  en: GOV_MATRIZ_CORRELACAO_en.md
  es: GOV_MATRIZ_CORRELACAO_es.md
  pt: GOV_MATRIZ_CORRELACAO.md
type: Governance
version: '2.1'
---

# Correlation Matrix: GDocs vs GitHub

**Series:** Governance **Version:** 2.0 | **Date:** 2026-03-04 | **Author:** Fabio Takwara | **License:** CC BY 4.0 | **DOI:** 10.5281/zenodo.18882784

[![DOI](https://img.shields.io/badge/DOI-10.5281/zenodo.18882784-blue.svg)](https://doi.org/10.5281/zenodo.18882784)
[![License: CC BY 4.0](https://img.shields.io/badge/License-CC%20BY%204.0-lightgrey.svg)](https://creativecommons.org/licenses/by/4.0/)
[![🇧🇷 PT](https://img.shields.io/badge/🇧🇷-Portuguese-green)](./GOV_MATRIZ_CORRELACAO.md) [![🇺🇸 EN](https://img.shields.io/badge/🇺🇸-English-blue)](./GOV_MATRIZ_CORRELACAO_en.md) [![🇪🇸 ES](https://img.shields.io/badge/🇪🇸-Spanish-orange)](./GOV_MATRIZ_CORRELACAO_es.md)
![Status](https://img.shields.io/badge/status-V2.0%20(Advocacy%205.1)-blue)

---

This document maps the files shared in Google Drive (Takwara Nucleus) to their corresponding location in the GitHub repository, ensuring technical synchrony between the Production WG and the Platform development.

---

| GDocs Name (Takwara Core) | GitHub Location (Docs) | Responsible / Status |
| :--- | :--- | :--- |
| **FINANCIAL SUMMARY** | `99_RESTRITO/01_FINANCEIRO/OPS_resumo-financeiro.md` | Synchronized |
| **Executive Report: Innovations** | `06_ANAIS_MEMORIA/OPS_relatorio-executivo-inovacoes.md` | Synchronized |
| **Regenerative Amazon Platform v5.1** | `06_ANAIS_MEMORIA/TAK_plataforma-v5.1.md` | Master v5.1 |
| **Manual - PHYTOREMEDIATION...** | `01_TECNOLOGIA_TAKWARA/TAK_manual-fitorremediacao-credito-carbono.md` | Synchronized |
| **BAMBOO BIOECONOMY HANDBOOK...** | `01_TECNOLOGIA_TAKWARA/TAK_manual-bioeconomia-bambu.md` | Synchronized |

---

## Governance Observations
- **Master GitHub**: The GitHub repository is the official source for versioning and preservation for Zenodo.
- **GDocs Editing**: Changes made by the WG in GDocs must be reported or integrated into GitHub monthly or according to project milestones (e.g., COP30).
- **No Venom**: Any technical change must pass through the scrutiny of [`GEMINI.md`](../GEMINI.md).

*Last updated: 2026-03-04.*
