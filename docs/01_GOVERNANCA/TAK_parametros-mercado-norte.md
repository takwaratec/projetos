---
title: "Parâmetros de Referência Orçamentária: Médias Industriais Amazônia (2026)"
author: "Avaliador Industrial Takwara"
date: '2026-03-06'
doi: 10.5281/zenodo.18843828
---

# Parâmetros de Referência: Precificação Industrial Norte (Manaus/Rio Branco)

Para o cálculo de **Contrapartida de PI** e **CAPEX de Projetos**, utilizaremos as seguintes médias ponderadas obtidas via sondagem de mercado (incluindo custos de frete logístico, impostos locais e insalubridade de operação remota).

## 1. Tabela de Insumos (Material Bruto + Processamento)

| Item | Descrição | Unidade | Valor Estimado (R$) |
| :--- | :--- | :--- | :--- |
| **Aço Carbono A36** | Chapa/Perfil (já processado: corte/dobra) | kg | R$ 18,50 |
| **Aço Inox 304** | Chapa/Tubulação (processamento sanitário) | kg | R$ 42,00 |
| **Poliuretano (PU)** | Matéria-prima base (Mamonex/Imperveg) | kg | R$ 35,00 |
| **Areia/Insumos Locais**| Agregados para biocompósitos | m³ | R$ 120,00 |

## 2. Tabela de Mão de Obra e Serviços Técnicos

| Especialidade | Nível de Qualificação | Unidade | Valor Hora (R$) |
| :--- | :--- | :--- | :--- |
| **Caldeiraria Pesada** | Sênior (Solda TIG/MIG) | hora | R$ 85,00 |
| **Tornearia e Usinagem** | Precisão (Eixos/Mancais) | hora | R$ 110,00 |
| **Instalação Elétrica** | Automação/Variadores | hora | R$ 95,00 |
| **Engenheiro/Acompanhamento**| Fiscalização "In Loco" | dia-técnico | R$ 1.200,00 |

## 3. Fatores Adicionais (Mark-up Logístico)

*   **Fator Logístico (FL):** +15% sobre o custo de insumos (reflete o frete Manaus-Rio Branco/Interior).
*   **Margem de Contingência:** 5% para variações de câmbio em componentes importados (ex: inversores).
*   **Valuation de Soberania (VS):** Multiplicador de **1.4x** sobre o custo de fábrica. Este fator quantifica o ativo intangível (Invenção/Patente), cobrindo o risco de P&D e a ausência de royalties estrangeiros.

---
**🎋 Takwara — Soberania Técnica em Bioeconomia**
