---
author:
- affiliation: Universidade de Brasília / Núcleo Takwara
  name: Takwara, Fabio Resck
  orcid: 0000-0001-8815-3885
date: '2026-03-04'
doi: 10.5281/zenodo.18827106
language: es
license: CC BY 4.0
series: Gobernanza Operativa
title: 'Matriz de Correlación: GDocs vs GitHub'
translations:
  en: GOV_MATRIZ_CORRELACAO_en.md
  es: GOV_MATRIZ_CORRELACAO_es.md
  pt: GOV_MATRIZ_CORRELACAO.md
type: Gobernanza
version: '2.2.2'
---

# Matriz de Correlación: GDocs vs GitHub

**Serie:** Gobernanza **Versión:** 2.2.2 | **Fecha:** 2026-03-04 | **Autor:** Fabio Takwara | **Licencia:** CC BY 4.0 | **DOI:** 10.5281/zenodo.18827106

[![DOI](https://img.shields.io/badge/DOI-10.5281/zenodo.18827106-blue.svg)](https://doi.org/10.5281/zenodo.18827106)
[![Licencia: CC BY 4.0](https://img.shields.io/badge/Licencia-CC%20BY%204.0-lightgrey.svg)](https://creativecommons.org/licenses/by/4.0/)
[![🇧🇷 PT](https://img.shields.io/badge/🇧🇷-Portugués-green)](./GOV_MATRIZ_CORRELACAO.md) [![🇺🇸 EN](https://img.shields.io/badge/🇺🇸-Inglés-blue)](./GOV_MATRIZ_CORRELACAO_en.md) [![🇪🇸 ES](https://img.shields.io/badge/🇪🇸-Español-orange)](./GOV_MATRIZ_CORRELACAO_es.md)
![Status](https://img.shields.io/badge/status-V2.2.2%20(Advocacy%205.1)-blue)

---

Este documento mapea los archivos compartidos en Google Drive (Núcleo Takwara) con su ubicación correspondiente en el repositorio GitHub, garantizando la sincronía técnica entre el GT de Producción y el desarrollo de la Plataforma.

---

| Nombre en GDocs (Núcleo Takwara) | Ubicación en GitHub (Docs) | Responsable / Estatus |
| :--- | :--- | :--- |
| **RESUMEN FINANCIERO** | `99_RESTRITO/01_FINANCEIRO/OPS_resumo-financeiro.md` | Sincronizado |
| **Informe Ejecutivo: Innovaciones** | `06_ANAIS_MEMORIA/OPS_relatorio-executivo-inovacoes.md` | Sincronizado |
| **Plataforma Amazonia Regenerativa v5.1** | `06_ANAIS_MEMORIA/TAK_plataforma-v5.1.md` | Maestro v5.1 |
| **Manual - FITORREMEDICIÓN...** | `01_TECNOLOGIA_TAKWARA/TAK_manual-fitorremediacao-credito-carbono.md` | Sincronizado |
| **CARTILLA DE BIOECONOMÍA...** | `01_TECNOLOGIA_TAKWARA/TAK_manual-bioeconomia-bambu.md` | Sincronizado |

---

## Observaciones de Gobernanza
- **Master GitHub**: El repositorio GitHub es la fuente oficial para el versionamiento y preservación para el Zenodo.
- **Edición en GDocs**: Los cambios realizados por el GT en GDocs deben ser reportados o integrados en GitHub mensualmente o según los hitos del proyecto (ej: COP30).
- **Sin Veneno**: Cualquier cambio técnico debe pasar por el escrutinio de [`GEMINI.md`](../GEMINI.md).

*Última actualización: 2026-03-04.*
