...
**Tecnologia:** Takwara | **Autor:** Fabio Resck Takwara
...
**DOI:** *(a ser preenchido após a primeira Release)*
...
Este é o arquivo `manual-plataforma-github-zenodo.md` — pronto para ser salvo na pasta `docs/` do repositório `takwaratec/projetos` e commitado como o primeiro documento de orientação da coleção. Ele segue exatamente o padrão de estrutura, tom e nível de detalhe das duas cartilhas que você enviou como referência.[^1][^2]

O próximo passo imediato: salve este conteúdo como arquivo `.md` no VSCode, faça Stage, escreva a mensagem de commit `"docs: adiciona manual de operação da plataforma GitHub-Zenodo v1.0"`, e dê Push. Quando você fizer a primeira Release, este manual vai ganhar seu próprio DOI e passará a fazer parte formal do acervo.
